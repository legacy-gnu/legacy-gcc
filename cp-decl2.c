...
void
check_classfn (ctype, cname, function, flags)
     tree ctype, cname, function;
     int flags;
{
  tree fn_name = DECL_NAME (function);
     ...
     tree thing;
{
  ...
  assert (TREE_CODE (thing) == IDENTIFIER_NODE);
     ...
     tree decl;
{
  if (DECL_PENDING_INLINE_INFO (decl) != 0
      && ! DECL_PENDING_INLINE_INFO (decl)->deja_vu)
    {
      ...
      assert (DECL_SAVED_INSNS (decl) == 0);
      while (t)
	{
...
finish_file ()
{
 ...
 mess_up:
  ...
  if (flag_all_virtual == 2)
    {
      tree __sosDynError = default_conversion (lookup_name (get_identifier ("sosDynError"), 0));
	...
	entry = tree_cons (NULL_TREE, integer_zero_node, entry);
      ...
      TREE_CONSTANT (TREE_VALUE (zlink)) = 1;
	...
	entry = tree_cons (NULL_TREE, integer_zero_node, entry);
      ...
      TREE_CONSTANT (TREE_VALUE (zret)) = 1;
      ...
      while (decls)
	{
	  if (TREE_PUBLIC (decls)
		  ...
		  || TYPE_DYNAMIC (DECL_CLASS_CONTEXT (decls)) == 0))
	    {
	      entry = build (CONSTRUCTOR, zlink_type, NULL_TREE,
			     tree_cons (NULL_TREE,
					build1 (ADDR_EXPR, string_type_node,
						combine_strings (build_string (IDENTIFIER_LENGTH (DECL_ASSEMBLER_NAME (decls)),
									       IDENTIFIER_POINTER (DECL_ASSEMBLER_NAME (decls))))),
	...
	while (tags)
	  {
		...
		/* SOS currently only implements single inheritance,
		   ...
		   has a base class.  */
		tree base_name = TYPE_BINFO_BASETYPES (type) && TYPE_DYNAMIC (TYPE_BINFO_BASECLASS (type, 0))
		  ? build1 (ADDR_EXPR, string_type_node, CLASSTYPE_TYPENAME_AS_STRING (TYPE_BINFO_BASETYPE (type, 0)))
		    : null_string;
		tree dyn_table, dyn_entry = NULL_TREE, fns = TYPE_BINFO_VIRTUALS (type);
		while (fns)
		  {
		...
		dyn_entry = tree_cons (NULL_TREE, build1 (NOP_EXPR, TYPE_POINTER_TO (default_function_type), sosVersionNumber),
				       tree_cons (NULL_TREE, integer_zero_node,
						  tree_cons (NULL_TREE, integer_zero_node,
							     dyn_entry)));
		dyn_table = finish_table (TYPE_IDENTIFIER (type), TYPE_POINTER_TO (default_function_type), dyn_entry, 0);
