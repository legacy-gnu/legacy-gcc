     ...
     tree decl;
{
  ...
  if (TYPE_DYNAMIC (type))
    vtbl = build1 (NOP_EXPR, ptr_type_node, lookup_name (get_identifier (AUTO_VTABLE_NAME), 0));
  else
#endif
    {
#if 1
     ...
     tree s_id, name, init;
{
  ...
  expand_member_init (build_indirect_ref (base), name, init);
}
  ...
  while (init_list)
      ...
      if (TREE_PURPOSE (init_list))
	{
	  if (TREE_CODE (TREE_PURPOSE (init_list)) == FIELD_DECL)
		...
		{
		  expand_expr_stmt (build_virtual_init (binfo, TREE_VALUE (init_list), addr));
     ...
     int use_global_new;
{
  ...
  if (placement == void_type_node)
      ...
      if (! IS_AGGR_TYPE (type))
	{
      ...
      else if (! is_aggr_typedef (TYPE_IDENTIFIER (type), 1))
	return error_mark_node;
      else
	 ...
	 we know where the object starts.  */
      objbits = build1 (INDIRECT_REF, unsigned_type_node,
			build (MINUS_EXPR, ptr_type_node,
			       rval, c_sizeof (unsigned_type_node)));
     ...
     tree auto_delete_vec, auto_delete;
{
  ...
  body = tree_cons (NULL_TREE,
		    build_delete (ptype, tbase, auto_delete,
				  LOOKUP_NORMAL|LOOKUP_DESTRUCTOR, 0, 0, 0),
		    body);
