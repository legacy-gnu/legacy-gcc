  ...
  if (instance == C_C_D)
    {
	  ...
	  || !DERIVED_FROM_P (DECL_FCONTEXT (CLASSTYPE_VFIELD (current_class_type)), basetype))
     ...
     tree fndecl, base_fndecl, pfn;
{
  ...
  if (context != t && TYPE_USES_COMPLEX_INHERITANCE (t))
    {
      ...
      if (offset == NULL_TREE)
	{
	  tree binfo = binfo_value (context, t, 0);
	  offset = BINFO_OFFSET (binfo);
	}
    }
  ...
  for (vbases = CLASSTYPE_VBASECLASSES (t); vbases; vbases = TREE_CHAIN (vbases))
	...
	{
	  tree context_binfo = binfo_value (base_context, base, 0);
     ...
     tree t; /* Structure type. */
{
  ...
  if (DECL_VINDEX (x) == error_mark_node)
	...
	if (i >= 256 || index_table[i] == 0)
	  {
	    index = build_int_2 (((unsigned)1 << (BITS_PER_WORD - 1)) | i, ~0);
	    if (i < 256)
	      index_table[i] = index;
	  }
	else
     ...
     tree type;
{
  ...
  for (i = n_baseclasses-1; i >= 0; i--)
	...
	continue;
      ...
      if (TYPE_HAS_CONSTRUCTOR (BINFO_TYPE (child)))
	{
	  /* ...and the last shall come first...  */
	  base_init_list = maybe_fixup_vptrs (type, child, base_init_list);
	  base_init_list = tree_cons (NULL_TREE, child, base_init_list);
	  continue;
	}

      if ((blist = CLASSTYPE_BASE_INIT_LIST (BINFO_TYPE (child))) == NULL_TREE)
	...
	continue;
	 ...
	 we are interested in.  */
	  ...
	  && tree_int_cst_equal (BINFO_OFFSET (child),
      ...
      else
	{
	  ...
	  if (base_init_list)
	    ...
	    base_init_list = CLASSTYPE_BINFO_AS_LIST (BINFO_TYPE (child));
	}
    }
  ...
  for (i = 0; i < n_baseclasses; i++)
	  ...
	  && BINFO_BASETYPES (child))
	{
	  ...
	  /* Completely unshare potentially shared data, and
	     update what is ours.  */
	  propagate_binfo_offsets (child, BINFO_OFFSET (child));
	}
	      ...
	      while (vfields)
		{
		  if (VF_BINFO_VALUE (vfields) == NULL_TREE
		      || ! TREE_VIA_VIRTUAL (VF_BINFO_VALUE (vfields)))
		    {
		      ...
		      b->vfields = tree_cons (child, value, b->vfields);
  ...
  if (n_baseclasses)
      ...
      for (; vbases; vbases = TREE_CHAIN (vbases))
	{
	  ...
	  if (CLASSTYPE_VSIZE (basetype))
		...
		if (DERIVED_FROM_P (basetype, this_binfo))
		  {
     ...
     int warn_anon;
{
  ...
  if (fn_fields)
	  ...
	  && DECL_FRIENDLIST (TYPE_NAME (t)) == NULL_TREE)
	{
	  ...
	  for (ctor = TREE_VEC_ELT (method_vec, 0); ctor;
      ...
      while (vbases)
	{
	  ...
	  while (virtuals != NULL_TREE)
	      ...
	      if (decl != base_fndecl && context != t)
		{
		  ...
		  unsigned i = (TREE_INT_CST_LOW (DECL_VINDEX (base_fndecl))
				& (((unsigned)1<<(BITS_PER_WORD-1))-1));
		  ...
		  if (binfo == NULL_TREE)
		    binfo = binfo_value (base_context, context, 0);
		  if (binfo != NULL_TREE)
		    {
  ...
  while (pending_hard_virtuals)
      ...
      else
	{
	  ...
	  assert (base_fndecls != error_mark_node);
	  while (base_fndecls)
	    {
	      modify_vtable_entries (t, TREE_PURPOSE (pending_hard_virtuals),
  ...
  if (pending_virtuals || CLASSTYPE_DOSSIER (t))
      ...
      else
	{
	  ...
	  if (! BINFO_NEW_VTABLE_MARKED (TYPE_BINFO (t)))
	    build_vtable (binfo_value (TYPE_BINFO_BASETYPE (t, first_vfn_base_index), t, 0), t);
  ...
  if (all_virtual == 2 && (max_has_virtual || method_vec))
	...
	{
	  tree inner;
	  for (inner = *outer; inner; inner = DECL_CHAIN (inner))
	    {
	      tree entry;
	      tree fn;
      ...
      if (TYPE_DYNAMIC (t))
	{
	  for (outer = &TREE_VEC_ELT (method_vec, 0);
	      ...
	      for (inner = *outer; inner; inner = DECL_CHAIN (inner))
		{
		  ...
		  TREE_STRING_LENGTH (str) = IDENTIFIER_LENGTH (DECL_ASSEMBLER_NAME (inner));
		  TREE_STRING_POINTER (str) = IDENTIFIER_POINTER (DECL_ASSEMBLER_NAME (inner));
		  TREE_CONSTANT (str) = 1;
		  TREE_STATIC (str) = 1;
		  TREE_TYPE (str)
  ...
  for (x = fields; x; x = TREE_CHAIN (x))
    {
      if (DECL_BIT_FIELD (x)
	  && TREE_CODE (TREE_TYPE (x)) == INTEGER_TYPE
	  && ((unsigned)TREE_INT_CST_LOW (DECL_SIZE (x))
  ...
  if (CLASSTYPE_VSIZE (t) != 0)
	...
	{
	  tree binfo = binfo_value (DECL_FIELD_CONTEXT (vfield), t, 0);
  ...
  if (DECL_CONTEXT (TYPE_NAME (t))
      && TREE_CODE (DECL_CONTEXT (TYPE_NAME (t))) == RECORD_TYPE)
